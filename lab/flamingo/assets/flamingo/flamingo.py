...
FLAMINGO_WHEEL_ACTUATOR_MLP_CFG = ActuatorNetMLPCfg(
    ...
    network_file=f"{FLAMINGO_ASSETS_DATA_DIR}/ActuatorNets/Flamingo/mlp/wheel/Flamingo_ActuatorNet_Wheel.pt",
    pos_scale=1.0,
    vel_scale=1.0,
    torque_scale=1.0,
    input_order="vel_pos",
...
FLAMINGO_WHEEL_ACTUATOR_LSTM_CFG = ActuatorNetLSTMCfg(
    ...
    input_order="vel_pos",
    effort_limit=5.0,  # taken from spec sheet
    velocity_limit=25.0,  # taken from spec sheet
    saturation_effort=5.0,  # same as effort limitd
)
...
FLAMINGO_CFG = ArticulationCfg(
    ...
    actuators={
        ...
        "joints": DelayedPDActuatorCfg(
            ...
            max_num_time_lags=4,  # physics time steps (max: 5.0 * 4 = 20.0ms)
            stiffness={
                ".*_hip_joint": 80.0,
                ".*_shoulder_joint": 80.0,
                ".*_leg_joint": 80.0,
            },  # default: 75.0
            damping={
                ".*_hip_joint": 0.65,
