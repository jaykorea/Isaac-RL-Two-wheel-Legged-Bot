...
parser.add_argument("--robot", type=str, default="koch", help="Name of the robot.")
...
class TableTopSceneCfg(InteractiveSceneCfg):
    ...
    if args_cli.robot == "a1":
        ...
        # cube.init_state.rot = [0.5, 0.5, 0.5, 0.5]
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    if use_relative_mode == True:
        ...
        ik_commands = torch.zeros(scene.num_envs, 6, device=robot.device)
    else:
        # pose (3) + orientation (4)
    ...
    while simulation_app.is_running():
        if count % 150 == 0:
            ...
            # x ∈ [0.3, 0.5]
            cube_state[:, 0] = 0.4 + 0.1 * torch.rand(scene.num_envs, device=robot.device)

            # y ∈ [-0.25, -0.15] U [0.15, 0.25]
            y_abs = 0.2 + 0.10 * torch.rand(scene.num_envs, device=robot.device)  # [0.15, 0.25]
            ...
            #* initial cube pose in world frame
            cube_pos_w = cube_state[:, 0:3]
            cube_quat_w = cube_state[:, 3:7]
            #* update to current cube pose (optional)
            ...
            if use_relative_mode:
                # 1) 현재 EE pose (base frame)
                ee_pos_b, ee_quat_b = math_utils.subtract_frame_transforms(
                    root_pose_w[:, 0:3], root_pose_w[:, 3:7],
                    ee_pose_w[:, 0:3], ee_pose_w[:, 3:7]
                )

                # 2) 목표 pose (base frame)
                goal_pos_b, goal_quat_b = math_utils.subtract_frame_transforms(
                    root_pose_w[:, 0:3], root_pose_w[:, 3:7],
                    cube_pos_w, cube_quat_w
                )

                # 3) Δpose 계산
                pos_err = goal_pos_b - ee_pos_b
                _, aa_err = math_utils.compute_pose_error(
                    ee_pos_b, ee_quat_b,
                ...
                # 4) command ← Δ
                ik_commands[:, 0:3] = pos_err
                ik_commands[:, 3:6] = aa_err

                # 5) 현재 EE pose 전달
                ik_action.set_command(ik_commands, ee_pos_b, ee_quat_b)
            else:
            ...
            # ④ Threshold 판단
            GRIPPER_THRESHOLD = 0.005  # 0.5 cm
        ...
        if use_relative_mode:
            goal_marker.visualize(cube.data.root_state_w[:, 0:3] + scene.env_origins, cube.data.root_state_w[:, 3:7])
        else:
