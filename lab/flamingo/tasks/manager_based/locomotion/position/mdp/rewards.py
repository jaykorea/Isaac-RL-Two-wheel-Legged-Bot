...
def reward_smoothing_lin_vel_xy_exp(env: ManagerBasedRLEnv, command_name: str, max_distance : float = 6.0 ,max_vel : float=0.75, tau:float = 0.5, temperature : float = 4.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: RigidObject = env.scene[asset_cfg.name]
    pos_diff = env.command_manager.get_command(command_name)[:, :2]# - asset.data.root_pos_w[:,:2]
    dist = torch.norm(pos_diff, dim=1)
    desired_speed = max_vel * torch.tanh(dist / tau) # tanh
    actual_speed = torch.norm(asset.data.root_lin_vel_b, dim=1)

    diff = torch.square(actual_speed - desired_speed)
    reward = torch.exp(-temperature * diff)
...
def penalty_fly_feet(env:ManagerBasedRLEnv,
    ...
    penalty_left = torch.where(current_air_time[:,0]>threshold, 1, 0)
    penalty_right = torch.where(current_air_time[:,1]>threshold, 1, 0)
    
    penalty = current_air_time[:,0] * penalty_left + current_air_time[:,1] * penalty_right
    
    return penalty


def penalty_COM(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"))-> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
    com_offsets = asset.root_physx_view.get_coms()
    current_com = asset.data.root_com_pos_w
    
    print("com_offsets : ", com_offsets)
...
def reward_x_axis_move(env: ManagerBasedRLEnv, command_name: str, temperature : float = 4.0)-> torch.Tensor:
    ...
    y_diff = env.command_manager.get_command(command_name)[:, 1]
    reward = torch.exp(-temperature*torch.square(y_diff))
    return reward

def penalty_y_axis_move(env: ManagerBasedRLEnv, command_name: str)-> torch.Tensor:
    # reward for y axis error  
    y_diff = env.command_manager.get_command(command_name)[:, 1]
    #reward = torch.exp(-temperature*torch.square(y_diff))
    return torch.square(y_diff)


def lift_foot_reward_exp(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    sensor_cfg: SceneEntityCfg,
    command_name: str,
    temperature: float = 1.0,
    wheel_height : float = 0.118,
    threshold : float = 0.05,
    k : float = 2.0
) -> torch.Tensor:
    """Reward the lifting of the foot towards an optimal height."""
    asset: RigidObject = env.scene[asset_cfg.name]
    ...
    pos_diff = env.command_manager.get_command(command_name)[:, :2]# - asset.data.root_pos_w[:,:2]
    dist = torch.norm(pos_diff, dim=1)
    weight = 1.0 / (1.0 + k * dist)
    
    max_target_height = torch.max(sensor.data.ray_hits_w[..., 2], dim=1)[0]
    min_target_height = torch.min(sensor.data.ray_hits_w[..., 2], dim=1)[0]
    
    target_foot_height = max_target_height + wheel_height
    # Get the current z position of the robot's base
    left_foot_z  = asset.data.body_link_pos_w[:, asset_cfg.body_ids[0], 2]
    right_foot_z = asset.data.body_link_pos_w[:, asset_cfg.body_ids[1], 2]

    error_left_foot = torch.abs(target_foot_height -left_foot_z) 
    error_right_foot = torch.abs(target_foot_height -right_foot_z)
    
    reward = torch.exp(-temperature*error_left_foot) + torch.exp(-temperature*error_right_foot)
    return torch.where(max_target_height-min_target_height>=threshold,reward/2,0)


def reward_smoothing_lin_vel_forward_x_exp(env: ManagerBasedRLEnv, command_name: str, max_vel : float=0.75, tau:float = 0.5, temperature : float = 4.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: RigidObject = env.scene[asset_cfg.name]
    pos_diff = env.command_manager.get_command(command_name)[:, :2]
    dist = torch.norm(pos_diff, dim=1)
    desired_vel = max_vel * torch.tanh(dist / tau)
    actual_x_vel = asset.data.root_lin_vel_b[:, 0]
    diff = torch.square(actual_x_vel - desired_vel)
    reward = torch.exp(-temperature * diff)
    return torch.where(actual_x_vel >= desired_vel, -0.1, reward)
...
# def heading_command_error_abs(env: ManagerBasedRLEnv, command_name: str, temperature : float ,k : float) -> torch.Tensor:
#     """Penalize tracking orientation error."""
#     command = env.command_manager.get_command(command_name)
#     pos_diff = command[:, :3]
#     dist = torch.norm(pos_diff, dim=1)
#     weight = 1.0 / (1.0 + k * dist)
#     heading_b = command[:, 3]
#     return weight * torch.exp(-temperature*heading_b.abs())

def reward_ang_vel_z_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"))-> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
    penalty = torch.square(asset.data.root_ang_vel_b[:,2])
    return penalty

def penalty_roll_vel(env : ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: RigidObject = env.scene[asset_cfg.name]
    penalty = torch.square(asset.data.root_ang_vel_b[:,0])
    return penalty

def penalty_yaw_vel(env : ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: RigidObject = env.scene[asset_cfg.name]
    penalty = torch.square(asset.data.root_ang_vel_b[:,2])
    return penalty
