...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: RLTaskEnv):
        ...
        if not isinstance(env.unwrapped, RLTaskEnv):
            raise ValueError(
                f"The environment must be inherited from RLTaskEnv. Environment type: {type(env)}"
            )
        ...
        self.num_obs = self.unwrapped.observation_manager.group_obs_dim["policy"]
        self.prev_obs = None
        self.prev2_obs = None
    ...
    def step(
        ...
        stacked_obs = torch.cat(
            (obs[:, :31], self.prev_obs[:, :31], self.prev2_obs[:, :31]), dim=1
        )

        # Add the last 3 velocity observations
        velocity_obs = obs[
            :, -3:
        ]  # Assuming the last 3 observations are velocity-related
        final_obs = torch.cat((stacked_obs, velocity_obs), dim=1)

        # Ensure final_obs has 108 observations
        # assert final_obs.shape[1] == 108, f"Expected 108 observations, got {final_obs.shape[1]}"
        ...
        # Return the step information
        return final_obs, rew, dones, extras
