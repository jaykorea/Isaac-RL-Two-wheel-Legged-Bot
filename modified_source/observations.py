...
state_storage = defaultdict(lambda: {'current': None, 'previous': None, 'previous2': None})
...
def prev_base_lin_vel(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['base_lin_vel']['previous']


def prev_prev_base_lin_vel(env: BaseEnv) -> torch.Tensor:
...
def prev_base_ang_vel(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['base_ang_vel']['previous']


def prev_prev_base_ang_vel(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def prev_projected_gravity(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['projected_gravity']['previous']


def prev_prev_projected_gravity(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def prev_root_pos_w(env: BaseEnv) -> torch.Tensor:
    """Previous Asset root position in the environment frame."""
    return state_storage['root_pos_w']['previous']


def prev_prev_root_pos_w(env: BaseEnv) -> torch.Tensor:
...
def prev_root_lin_vel_w(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['root_lin_vel_w']['previous']


def prev_prev_root_lin_vel_w(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def prev_root_ang_vel_w(env: BaseEnv) -> torch.Tensor:
    """Previous root angular velocity in the asset's root frame."""
    return state_storage['root_ang_vel_w']['previous']


def prev_prev_root_ang_vel_w(env: BaseEnv) -> torch.Tensor:
...
def prev_joint_pos_rel(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['joint_pos_rel']['previous']


def prev_prev_joint_pos_rel(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
    return state_storage['joint_pos_rel']['previous2']


def joint_pos_rel_cos(env: BaseEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def prev_joint_pos_rel_sin(env: BaseEnv) -> torch.Tensor:
    """Previous joint positions of the asset as sine values."""
    return state_storage['joint_pos_rel_sin']['previous']


def prev_prev_joint_pos_rel_sin(env: BaseEnv) -> torch.Tensor:
...
def joint_pos_norm(env: BaseEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    update_state_storage('joint_pos_norm', current_value)
    return current_value


def prev_joint_pos_norm(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['joint_pos_norm']['previous']


def prev_prev_joint_pos_norm(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
    return state_storage['joint_pos_norm']['previous2']
...
def joint_vel_rel(env: BaseEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    current_value = asset.data.joint_vel[:, asset_cfg.joint_ids] - asset.data.default_joint_vel[:, asset_cfg.joint_ids]
    update_state_storage('joint_vel_rel', current_value)
    return current_value


def prev_joint_vel_rel(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['joint_vel_rel']['previous']


def prev_prev_joint_vel_rel(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def height_scan(env: BaseEnv, sensor_cfg: SceneEntityCfg, offset: float = 0.5) -> torch.Tensor:
    ...
    current_value = sensor.data.pos_w[:, 2].unsqueeze(1) - sensor.data.ray_hits_w[..., 2] - offset
    update_state_storage('height_scan', current_value)
    return current_value


def prev_height_scan(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['height_scan']['previous']


def prev_prev_height_scan(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def prev_body_incoming_wrench(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['body_incoming_wrench']['previous']


def prev_prev_body_incoming_wrench(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
...
def last_action(env: BaseEnv, action_name: str | None = None) -> torch.Tensor:
    ...
    """
    current_value = env.action_manager.action if action_name is None else env.action_manager.get_term(action_name).raw_actions
    update_state_storage('last_action', current_value)
    return current_value


def prev_last_action(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['last_action']['previous']


def prev_prev_last_action(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
    return state_storage['last_action']['previous2']


def delayed_last_action(env: BaseEnv, action_name: str | None = None) -> torch.Tensor:
    """The last input action to the environment with delay."""
    current_value = env.action_manager.action if action_name is None else env.action_manager.get_term(action_name).raw_actions
    # Apply delay
    delay = torch.rand((env.num_envs, 1), device=env.device) # 0~1
    delayed_action = (1 - delay) * current_value + delay * (state_storage['last_action']['previous'] if state_storage['last_action']['previous'] is not None else current_value)
    # Update state storage
    update_state_storage('last_action', delayed_action)
    return delayed_action


def prev_delayed_last_action(env: BaseEnv) -> torch.Tensor:
    """Previous delayed action."""
    return state_storage['last_action']['previous']


def prev_prev_delayed_last_action(env: BaseEnv) -> torch.Tensor:
...
def prev_generated_commands(env: BaseEnv) -> torch.Tensor:
    """Previous root linear velocity in the asset's root frame."""
    return state_storage['generated_commands']['previous']


def prev_prev_generated_commands(env: BaseEnv) -> torch.Tensor:
    """Previous-previous root linear velocity in the asset's root frame."""
