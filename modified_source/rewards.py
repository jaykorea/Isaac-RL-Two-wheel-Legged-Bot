...
def joint_target_deviation_l1(
    env, target_joint_angle: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize joint positions that deviate from the default one."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    # compute out of limits constraints
    target_joint_pos = asset.data.default_joint_pos.clone()
    target_joint_pos[:, asset_cfg.joint_ids] = target_joint_angle
    angle = (
        asset.data.joint_pos[:, asset_cfg.joint_ids]
        - target_joint_pos[:, asset_cfg.joint_ids]
    )
    return torch.sum(torch.abs(angle), dim=1)
