...
class FLA_STAND(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, env_id='FLA_STAND-v0',
                 model_path='./assets/flamingo_tester.xml',
                 frame_skip=1, render_mode='human'):
        self.model_path = os.path.join(os.path.dirname(__file__), model_path)
        self.frame_skip = frame_skip
        self.render_mode = render_mode
        self.dt_ = 0.01  # 시뮬레이션 주기
        self.sim_duration = 21  # 시뮬레이션 시간 unit: sec
        ...
        self.step_frequency = 2 / self.dt_
...
#        pygame.display.flip()
    ...
    def step(self, action):
        ...
        action_scaled = np.zeros(8)
        action_scaled[:6] = 0.0

        if self.step_counter % self.sim_step < self.step_frequency:
            action_scaled[2] = -0.12
            action_scaled[3] = -0.12
            action_scaled[4] = 0.4
            action_scaled[5] = 0.4
            action_scaled[6] = 0.0
            action_scaled[7] = 0.0
        elif self.step_counter % self.sim_step < 2 * self.step_frequency:
            action_scaled[:6] = 0.0
            action_scaled[6] = 0.0
            action_scaled[7] = 0.0
        elif self.step_counter % self.sim_step < 4 * self.step_frequency:
            action_scaled[2] = -0.12
            action_scaled[3] = -0.12
            action_scaled[4] = 0.4
            action_scaled[5] = 0.4
            action_scaled[6] = 0.0
            action_scaled[7] = 0.0
        elif self.step_counter % self.sim_step < 6 * self.step_frequency:
            action_scaled[:6] = 0.0
            action_scaled[6] = 0.0
            action_scaled[7] = 0.0
        else:
        ...
        obs = self._get_obs(action_scaled)
        ...
        return obs, reward, done, term, {}, action_scaled
    ...
    def run_mujoco(self, session, input_name, env, sim_hz=100, decimation=1):
        """
        ...
        while True:
            ...
            for step in tqdm(range(int(self.sim_step)), desc="Inferencing...", disable=True):
                ...
                obs, rewards, dones, term, info, action_step = env.step(action_clipped)

                # Log the desired and actual values
                log_data['time'].append(current_time)
                log_data['desired_joint_positions'].append(action_step[:6])
                log_data['desired_wheel_velocities'].append(action_step[6:8])
...
if __name__ == '__main__':
    ...
    mujoco_thread = threading.Thread(target=env.run_mujoco, args=(session, input_name, env, 100, 1))
    mujoco_thread.start()
